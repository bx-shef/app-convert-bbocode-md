...
| ~~Мягкий retry/backoff на чистый сетевой обрыв в `useB24Rest`~~ — **проверено (5 ролей ревью): не добавляем.** HTTP-слой SDK (`core/http/limiters/manager.mjs` + `abstract-http.mjs`, @bitrix24/b24jssdk 2.x) уже ретраит и сетевой обрыв (`retryOnNetworkError: true` по умолчанию), и таймаут `408`, и `429`/`OPERATION_TIME_LIMIT`, и `503`/`QUERY_LIMIT_EXCEEDED` (`maxRetries: 3`, `retryDelay: 1000ms`, экспоненциальный backoff+jitter); наружу летят только настоящие 4xx. Свой внешний retry сбросил бы счётчик попыток SDK и шёл бы вслепую мимо его backoff/лимит-состояния, добавляя нагрузку на уже зарейтлимиченный метод; `*.update` идемпотентны по значению поля, но слепой ретрай может задвоить побочные эффекты (история правок, уведомления) при «применилось, но ответ потерян». Зафиксировано комментарием в `useB24Rest.ts` + guard-тестом | P2 | S | 🚫 (SDK покрывает) |
| Прикладной **таймаут/отмена** REST-вызова (`useB24Rest`/`index.vue`): SDK ретраит транзиентные ошибки, но верхнюю границу ожидания одного логического вызова не задаёт — при `OPERATION_TIME_LIMIT` встроенный retry может держать вызов до ~10 мин; сейчас `saveToB24`/`loadFromB24` обёрнуты только булевым `b24Busy`, без `AbortSignal`/wall-clock/кнопки отмены (в худшем случае кнопка «висит» без обратной связи). Прецедент — `useFeedback.ts` (`AbortSignal.timeout`). **Отдельная забота от ретраев** (замечание ревью к #106) | P2 | S | ⬜ |
| ИБ-заметка (форвард): если `buildLogger()`/`setLogger()` когда-либо подключат к `$b24` — держать прод-порог логов `WARNING`+ **или** явно исключать текстовые поля (`DESCRIPTION`/`COMMENT`/`POST_MESSAGE`/`fields`) из логируемых params наравне с секретными ключами (`redact.mjs` фильтрует по ключу, но контент пользователя как класс не защищает). Сейчас логгер = `NullLogger` (dead code), утечки нет | P3 | S | 📝 |
