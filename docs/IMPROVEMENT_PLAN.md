...
| ~~Мягкий retry/backoff на чистый сетевой обрыв в `useB24Rest`~~ — **проверено: не добавляем.** HTTP-слой SDK (`core/http/limiters/manager.mjs` + `abstract-http.mjs`) уже ретраит и сетевой обрыв (`retryOnNetworkError: true` по умолчанию), и таймаут `408`, и лимиты `429`/`OPERATION_TIME_LIMIT`, и перегруз `503`/`QUERY_LIMIT_EXCEEDED` (`maxRetries: 3`, `retryDelay: 1000ms`, адаптивный backoff+jitter); наружу летят только настоящие 4xx. Свой внешний retry дублировал бы попытки и рисковал бы двойной записью на неидемпотентных `*.update`-сохранениях. Зафиксировано комментарием в `useB24Rest.ts` | P2 | S | 🚫 (SDK покрывает) |
