...
- ⛔ **блокер** — заблокировано внешним условием
- ⬜ **не начато**
...
|---|---|---|---|
| Публиковать образ **только после зелёного CI** (`workflow_run:[ci]` + гейт `conclusion==success`; образец — `deploy.yml` job `verify-ci`). Сейчас `deploy-docker.yml` пушит `:latest` независимо от `ci.yml` | **P0** | M | ⬜ |
...
| Пункт | Приоритет | Сложн. | Статус |
|---|---|---|---|
| Оставить Cloudflare Web Analytics (cookieless) как базу; добавить `NUXT_PUBLIC_CF_ANALYTICS_TOKEN` в `.env.example` | **P0** | S | ⬜ |
...
Механика примера: `POST /feedback` (фрейм-токен-аутентификация, `kind` up/down +
...
**Фаза 1 — гигиена деплоя:** гейт «образ только после зелёного CI» · smoke-скрипт ·
CSP-хеши вместо `unsafe-inline`.
