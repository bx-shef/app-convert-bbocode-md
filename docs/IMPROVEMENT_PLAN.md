...
1. **Канал обратной связи** → ✅ **свой воркер + приватный репо-бакет**
   ...
   GitHub-issue-native, триаж агентом). Не Formspree. *Со стороны владельца:*
   создать приватный репо-бакет + задеплоить воркер с токеном (токен **не** в SPA-бандл).
...
3. **Продуктовые метрики** → ✅ **«заводи, предлагай сам»** → выбрана **Яндекс.Метрика
   ...
   *Со стороны владельца:* дать id счётчика в `NUXT_PUBLIC_YANDEX_COUNTER_ID`.
