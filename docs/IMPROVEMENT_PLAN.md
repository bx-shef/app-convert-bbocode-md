...
| **Runtime iframe-guard аналитики** (client-plugin: не инжектить CF-beacon при `window.self !== window.top`) — принцип №4 в коде. CF-beacon сторонний, сам себя не глушит → гейт на инъекции. **Нужен ДО включения токена** | P2 | S | ⬜ |
