...
## Установка
- [x] По умолчанию ставится только BBCode placement
- [x] Миграция: при ре-инсталле автоматически отвязывается legacy `/widget/im-keyboard`
- [ ] ~~Quick commands — опциональный чекбокс на install-step~~ → перенесено в страницу настроек админа (см. ниже)

## Страница настроек админа (новый пункт)
- [ ] `app/pages/settings.vue` — только для админа портала (`profile.ADMIN === true`)
- [ ] Тогл «Quick» (вкл/выкл `placement.bind` для `/widget/quick`)
- [ ] Confirm-диалог при выключении: «Убрать кнопку Quick из чата? Изменение применится для всех пользователей портала.»
- [ ] Источник истины — `placement.list` (Bitrix24 не отдаёт чужие placement'ы)
- [ ] Будущее место для редактора команд (текст / иконка / кому показывать)
- [ ] Ссылка в sidebar (`app/layouts/default.vue`), видна только админу
