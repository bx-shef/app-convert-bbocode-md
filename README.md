...
- **Поддерживаемые теги (базовый набор Bitrix24):** `b, i, u, s, url, img, list (+[*], [list=1]), code (+lang), quote, h1-h6, br, hr, p, table (tr/th/td), color, size, font, user, department`.
...
**Готово**
- Конвертер BBCode ⇄ Markdown ⇄ HTML + живое превью, печать, HTML-санитайзер; стилевые теги `[color]`/`[size]`/`[font]` и упоминания `[USER]`/`[DEPARTMENT]`.
...
Под превью есть кнопка «Сообщить об ошибке конвертации». Конвертер детерминирован,
поэтому исходная разметка = идеальный репро. Кнопка **скрыта**, пока не задан
эндпоинт:
