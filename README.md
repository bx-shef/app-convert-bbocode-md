...
- **Поддерживаемые теги (базовый набор Bitrix24):** `b, i, u, s, url, img, list (+[*], [list=1]), code (+lang), quote, h1-h6, br, hr, p, table (tr/th/td), color, size, font`.
...
**Готово**
- Конвертер BBCode ⇄ Markdown ⇄ HTML + живое превью, печать, HTML-санитайзер; стилевые теги `[color]`/`[size]`/`[font]`.
...
**Дальше**
- Теги-сущности: `[USER=id]`, `[DISK File=id]`, `[DEPARTMENT=id]` (рендер имён требует данных портала).
...
- [ ] Теги-сущности: `[USER=id]`, `[DISK File=id]`, `[DEPARTMENT=id]` — пока проходят как текст (round-trip без потерь, но не рендерятся).
